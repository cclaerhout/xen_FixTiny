/**
 * editor_plugin_src.js MODIFIED FOR XENFORO 3.4.x/3.5.x
 *
	...
	tinymce.create('tinymce.plugins.AutoSave', {
		...
		init : function(ed, url) {
			...
			ed.addButton(RESTORE_DRAFT, {
				title : ed.getLang('xenforo.restoredraft_desc'),
				onclick : function() {
					ed.focus(); //FIX
	      				if (ed.getContent({draft: true}).replace(/\s|&nbsp;|<\/?p[^>]*>|<br[^>]*>/gi, "").length > 0) {
	      					// For XenForo: use the default Javascript Confirm function => it will be faster to display than to get a XenForo template
	      					if (confirm(ed.getLang('xenforo.restoredraft_warning'))) {
	      						self.restoreDraft();
	      					}
	      				} else
	      					self.restoreDraft();
				...
				if (ed.controlManager.get(RESTORE_DRAFT)) {
					...
					setInterval(function() {
						if (!ed.removed) {
							...
							if(ed.getParam("autosave_display_in_statusbar")){
								var row = tinymce.DOM.get(ed.id + '_path_row'), 
								content = ed.getContent();
	
								if (row && content.length > ed.settings.autosave_minlength) {
									if(!$('#'+ ed.id + '_statusbar_autosave').length) {
										tinymce.DOM.add(row.parentNode, 'div', {'id': ed.id + '_statusbar_autosave', 'class': 'statusbar_autosave'}, 
										ed.getLang('xenforo.autosave_statusbar_msg'));
									}
	
									$('#'+ ed.id + '_statusbar_autosave').slideUp(300).fadeIn('slow').delay(1000).fadeOut(300);
								}
							}
							/* XenForo modif ends */
						}
			...
			_beforeUnloadHandler : function(e) {
				...
				tinymce.each(tinyMCE.editors, function(ed) {
					...
					if (!msg && ed.isDirty() && ed.getParam("autosave_ask_before_unload"))
						msg = ed.getLang('xenforo.autosave_unload_msg')
				});
