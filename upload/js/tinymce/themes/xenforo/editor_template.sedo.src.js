/*
 * Fast jQuery Tools overlay instead of slow TinyMCE inline popups
 */
(function($) {
	XenForo.tinymce = {
		_overlay_callbacks : function(src, ontrigger, onload) {
			XenForo.tinymce.t = src;
			XenForo.tinymce.ontrigger = ontrigger;
			XenForo.tinymce.onload = onload;
			...
			XenForo.tinymce.ed = ed;
			XenForo.tinymce.sel = ed.selection;
			XenForo.tinymce.bm = ed.selection.getBookmark();
		},
		_overlay_success: function(ajaxData)
		{
			var t = XenForo.tinymce.t,
						...
						if(isTouch) { overtop = '10%'; } else { overtop = 'center'; }
		  				...
		  				overlay = $(ajaxData.templateHtml)
							...
							.overlay({
								...
								onLoad: function(e) {
									$myOverlay = this.getOverlay().insertAfter('#exposeMask').addClass('isLoaded');//@http://stackoverflow.com/questions/1939494/
									$trigger = $myOverlay.find('.tinyTrigger');
									$focus =  $myOverlay.find('.mceFocus');
									$inputs = $myOverlay.find('input,textarea,select')


									//Hook onload
									if (typeof XenForo.tinymce.onload != 'undefined')
									{
										t[XenForo.tinymce.onload](XenForo.tinymce.sel, $myOverlay);
									}
										...
										$trigger.one('click', function (e){
											...
											ed.selection.moveToBookmark(XenForo.tinymce.bm);

											if (typeof XenForo.tinymce.ontrigger != 'undefined')
											{
												//Hook ontrigger
												t[XenForo.tinymce.ontrigger](inputs);
											}
										...
										//Fix Chrome bug with BookMark: http://xenforo.com/community/threads/tinymce-fix.39387/page-8#post-462736
...
(function() {
	...
	tinymce.create('tinymce.plugins.XenForoMedia', {
			...
			button = controlManager.createButton('xenforo_media', {
				...
				onclick : function(ui, val)
				{
					if (ed.getParam('fast_overlay') == 1) {
						XenForo.tinymce._overlay_callbacks(src, '_media_ontrigger', '_media_onload');
  						XenForo.ajax('index.php?editor/fast-dialog',	{ dialog: 'fast_media'}, XenForo.tinymce._overlay_success);
  					}
  					else{
		...
		_media_ontrigger : function(inputs)
		{
			XenForo.ajax(
		...
		_media_insert : function(ajaxData)
		{
		...
		_media_onload : function(selection, $overlay)
		{
			var url = selection.getContent()
			url = XenForo.tinymce._unescapeHtml(url, 'space');

			if(url.length > 0)
			{
				$overlay.find('#ctrl_url').val(url);	
			}
		},
		getInfo : function()
		{
	...
	tinymce.create('tinymce.plugins.XenForoCode', {
		addButtons : function (theme, tb)
		{
			...
			button = controlManager.createButton('xenforo_code', {
				...
				onclick : function(ui, val)
				{
					...
					if (ed.getParam('fast_overlay') == 1) {
						XenForo.tinymce._overlay_callbacks(src, '_code_ontrigger', '_code_onload');
	  					XenForo.ajax('index.php?editor/fast-dialog',	{ dialog: 'fast_code'}, XenForo.tinymce._overlay_success);
	  				}
	  				else {
						ed.windowManager.open({
		...
		_code_ontrigger : function(inputs)
		{
			var ed = XenForo.tinymce.ed, tag, code, output;
			...
			code = XenForo.tinymce._escapeHtml(inputs.code, 'space');
		...
		_code_onload : function(selection, $overlay)
		{
			/*
				//UnescapeHtml function not enough => there are still some html elements
				//=> need a function to convert html to raw text (may be the parser has got one - I didn't check)
				
				var string = selection.getContent()
				string = XenForo.tinymce._unescapeHtml(string, 'space');
				
				if(string.length > 0)
				{
					$overlay.find('#ctrl_code').val(string);	
				}
			*/
		},
		getInfo : function()
...
//				//minHeight = $(ed.getWin()).height(); //Creates an oversized overlay window on Chrome & creates problem with Firefox when content is only 1-2 lines
				minHeight =  parseInt($iframe.css('height')); //Iframe height is now generated dynamically with TinyMCE 1.5.7
...
*/
		...
		_mceImage : function(ui, val) {
			...
			if (ed.getParam('fast_overlay') == 1) {
				XenForo.tinymce._overlay_callbacks(this, '_mceImage_ontrigger', '_mceImage_onload');
	  			XenForo.ajax('index.php?editor/fast-dialog',	{ dialog: 'fast_image'}, XenForo.tinymce._overlay_success);
	  		}
	  		else {
				ed.windowManager.open({
		...
		_mceImage_onload : function(selection, $overlay)
		{
			var ed = this.editor,
			src = ed.dom.getAttrib(selection.getNode(), 'src');

			if(src.length > 0)
			{
				$overlay.find('#ctrl_src').val(src);	
			}
		},
		_mceImage_ontrigger : function(inputs)
		{
			var t = this, ed = this.editor, args = {}, el;
			if (inputs.src === '') {
		...
		_mceLink : function(ui, val) {
			...
			if (ed.getParam('fast_overlay') == 1) {
				XenForo.tinymce._overlay_callbacks(this, '_mceLink_ontrigger', '_mceLink_onload');
	  			XenForo.ajax('index.php?editor/fast-dialog', { dialog: 'fast_link'}, XenForo.tinymce._overlay_success);
	  		}
	  		else {
				ed.windowManager.open({
		...
		_mceLink_onload : function(selection, $overlay)
		{
			var ed = this.editor;
	
			if (e = ed.dom.getParent(ed.selection.getNode(), 'A')) {
				$overlay.find('#ctrl_url').val(ed.dom.getAttrib(e, 'href'));
			}
		},
		_mceLink_ontrigger : function(inputs)
		{
			var ed = this.editor, e, b, href = this._mceLink_checkPrefix(inputs.href).replace(/ /g, '%20');
			...
			if (e && (e.childNodes.length != 1 || e.firstChild.nodeName != 'IMG')) {
				...
				//ed.execCommand('mceStoreSelection'); //Do activate this =>only on popup I guess
			}

			ed.execCommand("mceEndUndoLevel");
		},
		_mceLink_checkPrefix : function(n) {
			n = $.trim(n);
	
			var primaryValue = n.replace(/\?.*$/, '');
	
			if (n && Validator.isEmail(primaryValue) && !/^\s*mailto:/i.test(n))
			{
				n = 'mailto:' + n;
			}
			else if (!/^((https?|ftp):\/\/|mailto:)/i.test(n))
			{
				n = 'http://' + n;
			}
			return n;
